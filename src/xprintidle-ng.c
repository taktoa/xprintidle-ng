...
static ulong workaroundX11(Display *dpy, ulong _idleTime);
...
static ulong get_idle_time() {
    // Open display
    open_display();

    // Allocate XScreenSaverInfo variable
    XScreenSaverInfo* ssi = XScreenSaverAllocInfo();

    // Pointers for event and error results
...
static void print_idle_time() {
    fmbprint(stdout, timefmt(time_format, get_idle_time()));
}
...
int main(int argc, char* argv[]) {
    ...
    char* time_format_raw = _(TIME_FMT_DEFAULT);
